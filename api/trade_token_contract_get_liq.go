...
func (api *Api) TradeTokenContractGetLiq(args *TradeTokenContractGetLiqArgs, result *string) error {
	...
	check := validateGetLiq(args.Mnemonic, args.TokenLabel, uwAddress, args.ScAddress)
	if check != 0 {
...
func validateGetLiq(mnemonic, tokenLabel, uwAddress, scAddress string) int64 {
	if mnemonic == "" {
		return 1
	}
	...
	validateGetLiq := trade_token_con.ValidateGetLiq(trade_token_con.NewGetArgsForValidate(scAddress, uwAddress, tokenLabel))
	if validateGetLiq != 0 {
		return validateGetLiq
	}

	validateTxInMemory := validateTxInMemory(uwAddress, scAddress, "trade_token_contract_get_liq_transaction", 1)
