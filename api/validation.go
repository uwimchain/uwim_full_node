...
func validateBalanceTest(address string, amount float64, tokenLabel string, taxNull bool) *api_error.ApiError {
	balance := storage.GetBalance(address)
	if balance == nil {
...
func validateTxInMemoryTest(from, to, txType string, value int64) *api_error.ApiError {
	var txValue int64 = 0
